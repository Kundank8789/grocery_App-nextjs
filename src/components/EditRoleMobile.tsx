...
function EditRoleMobile() {
  ...
  const handleEdit = async () => {
    try {
      const result = axios.post("/api/user/edit-role-mobile", {
        role: selectedRole,
        mobile
      })
  ...
  useEffect(() => {
    const checkForAdmin = async () => {
      try {
        const result = await axios.get("/api/admin/check-for-admin");
        if(result.data.adminExit){
          ...
          return (
            <motion.div
              ...
              className={`flex flex-col items-center justify-center w-48 h-44 rounded-2xl border-2 transition-all ${isSelected
                  ? "bg-green-600 by-green-100 shadow-lg"
